 ...
 function sort_up(data){ 
	var data_after=[]; 
		for(var i=0;i<data.length;i++){
			data_after[i]={ uopenid:"",uid:"",uname:"",upasswd:"",urealname:"",usex:"",uage:"",
			                utel:"",ucmeal:"",ucpay:"",uconsume:"",
						    uscore:"",umoney:"",uactivity:"",utime:"",umark:"",sid:"",}	
		}	
	...
	for(var i=0;i<data.length;i++){
		 for(var j=0;j<data1.length;j++){
			 ...
			 data_after[j].upasswd=data[i].upasswd;
			 data_after[j].urealname=data[i].urealname;
			 ...
			 data_after[j].sid=data[i].sid;
			 }			 
		 }	
	}	 
	 var cs = new table({
        "tableId":"cs_table",    //必须		 
        "headers":["微信id","用户id","姓名","密码","真实姓名","性别" ,"年龄","电话",
		          "就餐次数","付款次数","累计消费","积分","账户余额","活跃度" ,"注册时间","备注","商家"],   //必须
        "data":data_after,        //必须
 ...
 function sort_down(data){ 
    ...
    $.ajax({
		...
		dataType: "jsonp",	 
		data: {sql: "select * from user"},    //附加数据请用json表示
		type: 'get',
